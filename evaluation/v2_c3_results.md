# Chunking Config 3 Evaluation Results
...
Retrieval-Augmented Generation (RAG) addresses the problem of enabling language models to reliably use up-to-date, inspectable external knowledge for knowledge‑intensive tasks (e.g., open-domain QA or factual generation) by combining a pre-trained seq2seq model with a retrieval-based non‑parametric memory (a dense index) [Source 1], [Source 3].

A model’s parametric knowledge (what’s stored in its weights) can be insufficient because it is hard to access and precisely manipulate, it can’t be easily expanded or revised, it struggles to provide provenance for its outputs, and it may “hallucinate” facts—leading to weaker performance on knowledge‑intensive tasks compared to retrieve‑and‑generate approaches [Source 1], [Source 3]. RAG therefore augments parametric memory with retrieval to improve factuality, updateability, and interpretability [Source 2].
...
RAG pairs a pre-trained seq2seq transformer as parametric memory with a dense-vector index (Wikipedia) as non‑parametric memory. A neural retriever (DPR) returns latent documents z conditioned on the input x, and the seq2seq generator (BART) conditions on the input plus those retrieved documents to produce the output [Source 2]. The model is formulated probabilistically and trained end-to-end, marginalizing over the latent documents with a top-K approximation—either per-output (same document for all tokens) or per-token (different documents for different tokens) [Source 2]. RAG thus aggregates evidence from multiple retrieved pieces and allows the non‑parametric index to be swapped/updated without retraining the generator [Source 1][Source 3].
...
Short answer: RAG augments a parametric language model with a non‑parametric retrieval component that fetches relevant external passages at generation time, which makes it more factual, updatable, and interpretable on knowledge‑intensive tasks than a model that relies only on its parameters [Source 1], [Source 3].

Key differences (from the provided context)
- External, inspectable memory: RAG retrieves passages from a vector index (e.g., Wikipedia) and conditions generation on those retrieved passages, instead of depending solely on knowledge stored in model weights [Source 1].
- Better factuality and specificity: RAG’s generations are reported to be more factual and specific than a parametric-only baseline (BART) on knowledge‑intensive generation tasks [Source 3].
- Abstractive generation with retrieval: RAG can produce free‑form, abstractive answers (not just extractive spans) while using retrieved evidence to support generation [Source 2].
- Learnable, task‑aware retrieval: The retrieval component is differentiable and can be learned; learned retrieval improves performance (especially for open‑domain QA) compared to freezing the retriever or using simple word‑overlap (BM25) alone [Source 3].
- Easily updated knowledge: Because RAG stores knowledge non‑parametrically, the index can be swapped or updated at test time to change the model’s knowledge without re‑training the generator—something parametric‑only models require further training to achieve [Source 1], [Source 3].

If you want, I can summarize how these differences affect a specific QA or generation task.
...
- Retrieval/top-K: both models first retrieve a top-K set of documents pη(z|x) and use those as latent documents for generation [Source 1], [Source 2].

- RAG-Sequence: treats the retrieved document as a single latent variable for the entire output. For each candidate document z the generator scores the whole sequence conditioned on z, and the model marginalizes over documents (sum over documents of the sequence probability). Concretely, the same retrieved document is used to generate every token of the sequence [Source 1].

- RAG-Token: marginalizes per output token rather than per sequence. For each generation step the generator computes token distributions conditioned on each retrieved document and then marginalizes, so it can (effectively) draw a different document for different target tokens. This lets the model combine content from several documents when producing an answer [Source 2].

- Practical effect on documents used: RAG-Sequence bases the entire output on a single retrieved document (per marginalization term), whereas RAG-Token can mix evidence across multiple retrieved documents across tokens, enabling token-level switching among documents. (For a length‑one target—e.g., sequence classification—the two are equivalent.) [Source 1], [Source 2]
...
By pre‑training a very large autoregressive model (GPT‑3, 175 billion parameters) and then evaluating it in zero‑shot, one‑shot and few‑shot settings, the authors showed the model can follow natural‑language instructions and a small number of in‑prompt examples to perform new tasks at test time without any gradient updates or task‑specific fine‑tuning. Scaling the model to 175B parameters led to large improvements in this task‑agnostic, in‑context (prompted) few‑shot performance, in some cases approaching the accuracy of prior fine‑tuned systems [Source 1]. The study deliberately used a pure autoregressive setup to focus on this in‑context learning behaviour and applied GPT‑3 to all tasks without updating its weights [Source 2].
...
T5 casts every NLP problem as a text-in → text-out task (e.g., "translate English to German: ...", "summarize: ...", "cola sentence: ..."), so each problem becomes the same type of learning problem: generate target text from input text. Because of that unified formulation, the authors can use the same Transformer model, the same training objective/loss, the same hyperparameters and decoding procedure for all tasks (translation, QA, classification, summarization, etc.), letting one model and training framework be applied uniformly across diverse tasks [Source 3], [Source 2]. The work therefore provides a single testbed for comparing transfer-learning choices while scaling models and data [Source 2], and all models studied are based on the Transformer architecture used throughout the paper [Source 1].
...
Why LoRA thinks adaptation has low intrinsic rank
- Prior work shows over‑parameterized pretrained models live on a low intrinsic dimension, so LoRA hypothesizes that the weight changes made during adaptation also lie in a low‑rank subspace (i.e., the update matrix has low “intrinsic rank”) [Source 2].  
- Empirically, LoRA finds that very small ranks (r = 1 or 2) already perform competitively, and the learned subspaces for different r and random seeds overlap, suggesting the actual adaptation directions are low‑dimensional rather than filling the full parameter space [Source 3].

How LoRA exploits that observation
- Instead of fine‑tuning all weights, LoRA freezes the pretrained model and injects a pair of trainable low‑rank decomposition matrices (often denoted A and B) into each Transformer layer to parameterize only the low‑rank change to the dense weights. Those small matrices are the only trained parameters for a task [Source 2].  
- This lets LoRA represent the required weight update with far fewer parameters, enabling massive reductions in trainable parameters and GPU memory (e.g., orders‑of‑magnitude fewer trainable params and reduced memory for GPT‑3 experiments) while matching or exceeding full fine‑tuning performance and allowing cheap task‑specific modules to be swapped in/out [Source 1][Source 2].
...
ReAct interleaves verbal reasoning traces and environment actions: prompts the LLM to both "think" (generate reasoning steps, plans, and subgoal decompositions) and "act" (issue environment queries or operations) in a single, closed-loop trajectory. This lets the model perform dynamic reasoning to create, maintain, and adjust high-level plans (reason→act) while also using interactions with external sources (e.g., Wikipedia) to bring new evidence into its reasoning (act→reason) [Source 1].

This synergy yields clear advantages over reasoning-only approaches because:
- It lets the model incorporate external information into its chain of thought, improving decisions that depend on outside facts or observations [Source 1].  
- The combined approach produces more flexible, sparse, and diverse reasoning traces (including commonsense and goal decomposition) than approaches that only react to observations, which often fail to identify completed subgoals or next subgoals and to locate items in environments [Source 2].  
- Empirically, ReAct substantially outperforms an IM-style (reasoning-lite) baseline (71% vs. 53% overall success), showing consistent advantages on most tasks tested [Source 2].  
- It also improves diagnosability: humans can distinguish internal knowledge from externally retrieved information and inspect reasoning traces to understand action decisions—benefits absent when reasoning is done in isolation [Source 3].

In short, by tightly coupling reasoning and acting, ReAct can adapt plans based on external feedback and use environment interactions to strengthen its reasoning, yielding better performance and interpretability than reasoning-only methods [Sources 1–3].
...
- Chain-of-thought (CoT) prompting elicits explicit intermediate reasoning steps from a model, and this capability is an emergent property that appears only once the model is sufficiently large [Source 1], [Source 3].

- When CoT is present, those intermediate steps materially improve performance on complex reasoning tasks: tasks that show flat scaling with standard prompting exhibit dramatically increasing improvement when CoT is used [Source 1], [Source 3].

- There is a scale-dependent threshold: CoT can hurt performance for models below a certain size (notably for most models smaller than ~10B parameters), while scaling up fixes many error types (semantic misunderstandings, missing steps, etc.); for example, scaling PaLM up to 540B reduced a substantial portion of such errors [Source 2].

- CoT improvements arise from prompting an off‑the‑shelf model (no finetuning) and can also enable out‑of‑distribution generalization (e.g., to longer sequence lengths) [Source 1], [Source 3].
...
The authors note several limits that constrain RAG’s reliability and usefulness:

- Dependence on relevant (gold) passages: strong performance in their experiments relies on having access to gold passages that contain the specific information needed to generate the reference answer; many questions are unanswerable without those passages [Source 1]. This means RAG’s correctness depends heavily on retrieval quality—if the retriever fails to return the needed documents, RAG may be unable to produce a correct answer.

- Coverage limits of the retrieval corpus: not all questions are answerable from Wikipedia alone, so when the retrieval corpus lacks coverage of the needed facts, RAG cannot reliably answer those queries [Source 1]. In practice this reduces usefulness for knowledge not present in the search corpus.

- Residual hallucination: although RAG “hallucinates less and generate[s] factually correct text more often than BART,” hallucination is not eliminated entirely [Source 1]. Thus some generated answers can still be incorrect or misleading even if less frequently than with a purely generative baseline.

Together these limitations imply that RAG’s outputs are only as reliable as (a) the retriever’s ability to find relevant, evidence-bearing documents and (b) the coverage of the underlying corpus; gaps or retrieval failures can lead to unanswerable, incorrect, or incomplete generations [Source 1].
